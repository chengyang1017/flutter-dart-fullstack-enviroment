...
$androidDevices = @($devicesJson | Where-Object { $_.targetPlatform -like 'android-*' })
...
if ($env:FLUTTER_ANDROID_DEVICE) {
    $device = $androidDevices | Where-Object { $_.id -eq $env:FLUTTER_ANDROID_DEVICE } | Select-Object -First 1
    if (-not $device) {
