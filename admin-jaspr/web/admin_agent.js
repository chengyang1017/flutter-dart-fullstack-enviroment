(() => {
  ...
  const AGENT_MESSAGES_KEY = 'flutter_workbench_admin_agent_messages_v1';
  const AGENT_OPEN_KEY = 'flutter_workbench_admin_agent_open_v1';
  const MAX_MESSAGES = 40;

  const state = {
    open: sessionStorage.getItem(AGENT_OPEN_KEY) === '1',
  ...
  function syncContext() {
    ...
    if (course) {
      ...
      contextLabel.textContent = `${title} · ${currentEditLanguage().toUpperCase()}`;
      translateEnButton.hidden = false;
      translateZhButton.hidden = false;
    } else {
      contextLabel.textContent = `${currentSection()} · no course selected`;
  ...
  async function refreshStatus() {
    if (!token()) return;
    try {
      const result = await api('/admin/agent/status');
      state.status = result;
      renderStatus();
    } catch (error) {
      state.status = {
  ...
  function renderStatus() {
    ...
    if (!state.status) {
      statusLabel.textContent = 'Checking agent…';
      statusLabel.dataset.state = 'checking';
      return;
    }
    if (state.status.configured) {
      statusLabel.textContent = `${state.status.model || 'AI'} · ready`;
      statusLabel.dataset.state = 'ready';
      return;
    }
    statusLabel.textContent = state.status.error
  ...
  function renderMessages() {
    ...
    if (state.messages.length === 0) {
      ...
      empty.innerHTML =
        '<strong>Course assistant</strong><span>Ask about wording, localization or the current course. Use the quick actions for safe translation drafts.</span>';
  ...
  function renderProposal() {
    ...
    copy.textContent =
      'Nothing is saved yet. Apply it to the current course draft, review, then save the course JSON.';
    ...
    dismiss.addEventListener('click', dismissProposal);
  ...
  function openPanel() {
    state.open = true;
    sessionStorage.setItem(AGENT_OPEN_KEY, '1');
    syncOpenState();
    syncContext();
  ...
  function closePanel() {
    state.open = false;
    sessionStorage.setItem(AGENT_OPEN_KEY, '0');
    syncOpenState();
  }
  ...
  function mount() {
    ...
    setInterval(syncAuthState, 1200);
  }
